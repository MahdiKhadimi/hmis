...
<?php
   ...
   if(isset($_GET['news_id'])){
	...
	if(isset($_POST["title"])) {
		...
		$result =mysqli_query($con, "UPDATE news SET title='$title',description= '$description',news_file='$path',news_date='$news_date',source='$source' WHERE news_id=$news_id");
		
		if($result) {
			header("location:news_list.php?add=done");
		}
		else {
			header("location:news_edit.php?error=notadd&news_id=$news_id");
		}
	...
	<div class="panel-body">
		...
		<form method="post" enctype="multipart/form-data">
			...
			<div class="input-group">
				...
				<input value="<?php echo $row_news['title'];?>" required type="text" name="title" class="form-control">
			</div>
			...
			<img src="<?php echo $row_news['news_file']?>" style="width:80px;">
			<div class="input-group">
				<span class="input-group-addon">
					News Date
				</span>
				<input type="text" value="<?php echo $row_news['news_date']; ?>" autocomplete="off" name="news_date" class="form-control" id="newsDate">
			</div>
		    <div class="input-group">
				<span class="input-group-addon">
					Source:
				</span>
				<input type="text" value="<?php echo $row_news['source']; ?>" name="source" class="form-control">
			</div>
